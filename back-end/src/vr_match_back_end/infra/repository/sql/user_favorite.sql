...
-- :name favorited-from-user-without-matched-user-by-user_id :? :*
-- :doc user_idをお気に入りにしていて、ユーザーとマッチングしていないユーザーを取得
select
user_with_favorite_num.id,
user_with_favorite_num.name,
user_with_favorite_num.introduction,
...
inner join user_favorite
on user_with_favorite_num.id = user_favorite.from_id
and user_favorite.to_id = :user_id
where user_with_favorite_num.num = 1
and user_favorite.created_at < :after
order by user_favorite.created_at desc
limit :limit
